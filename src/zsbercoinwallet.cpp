...
CzsbercoinWallet::CzsbercoinWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsbercoin seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZsbercoinSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZsbercoinSeed_deprecated()) {
                    LogPrintf("%s: Updated zsbercoinseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsbercoin seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsbercoin wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzsbercoinWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZsbercoinCount(nCountLastUsed);
    else if (!walletdb.ReadZsbercoinCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzsbercoinWallet::Lock()
{
    seedMaster = 0;
}

void CzsbercoinWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzsbercoinWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZsbercoin(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzsbercoinWallet::LoadMintPoolFromDB()
{
...
void CzsbercoinWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzsbercoinWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzsbercoinWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsbercoinTracker->HasPubcoinHash(pMint.first)) {
...
bool CzsbercoinWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZsbercoin(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsbercoinTracker which also adds to database
    pwalletMain->zsbercoinTracker->Add(dMint, true);
...
void CzsbercoinWallet::SeedToZsbercoin(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzsbercoinWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzsbercoinWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZsbercoinCount(nCountLastUsed);
...
void CzsbercoinWallet::GenerateDeterministicZsbercoin(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzsbercoinWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzsbercoinWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
