...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZsbercoinStake(spend));
    } else {
        ...
        CsbercoinStake* sbercoinInput = new CsbercoinStake();
        sbercoinInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(sbercoinInput);
    }
