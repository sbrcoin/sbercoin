...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zsbercoinTracker = unique_ptr<CzsbercoinTracker>(new CzsbercoinTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == Zsbercoin_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == Zsbercoin_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
