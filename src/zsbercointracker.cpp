...
CzsbercoinTracker::CzsbercoinTracker(std::string strWalletFile)
{
...
CzsbercoinTracker::~CzsbercoinTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzsbercoinTracker::Init()
{
...
bool CzsbercoinTracker::Archive(CMintMeta& meta)
{
...
bool CzsbercoinTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzsbercoinTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzsbercoinTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzsbercoinTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzsbercoinTracker::GetSerialHashes()
{
...
CAmount CzsbercoinTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzsbercoinTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzsbercoinTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzsbercoinTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzsbercoinTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzsbercoinTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzsbercoinTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzsbercoinTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzsbercoinTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzsbercoinTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzsbercoinTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzsbercoinTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzsbercoinTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzsbercoinTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzsbercoinTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzsbercoinTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzsbercoinTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzsbercoin from DB\n", __func__, listDeterministicDB.size());
    }
