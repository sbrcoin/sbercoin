...
#include "zsbercoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zsbercoin ought to be enough for anybody." - Bill Gates, 2017
    ui->zsbercoinpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzsbercoinSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zsbercoinpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzsbercoin_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsbercoin is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zsbercoin, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zsbercoin...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zsbercoin in ") +
...
void PrivacyDialog::on_pushButtonSpendzsbercoin_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsbercoin is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zsbercoin, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zsbercoin
        sendzsbercoin();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zsbercoin
    sendzsbercoin();
}

void PrivacyDialog::on_pushButtonZsbercoinControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZsbercoinControlDialog* zsbercoinControl = new ZsbercoinControlDialog(this);
    zsbercoinControl->setModel(walletModel);
    zsbercoinControl->exec();
}

void PrivacyDialog::setZsbercoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzsbercoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzsbercoin()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid sbercoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zsbercoinpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zsbercoin is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " sbercoin </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zsbercoin</b>";
    ...
    // use mints from zsbercoin selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZsbercoinControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZsbercoinControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zsbercoin require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zsbercoin"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == Zsbercoin_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zsbercoin require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zsbercoin"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zsbercoin transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zsbercoin was spent successfully update the addressbook with the label
    ...
    // Clear zsbercoin selector in case it was used
    ZsbercoinControlDialog::setSelectedMints.clear();
    ui->labelzsbercoinSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zsbercoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " sbercoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zsbercoin Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zsbercoinpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsbercoinTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zsbercoin </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zsbercoin "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zsbercoin "));
    ui->labelzsbercoinAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zsbercoin </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zsbercoin </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzsbercoinSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzsbercoin->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zsbercoin
        ui->pushButtonMintzsbercoin->setEnabled(false);
        ui->pushButtonMintzsbercoin->setToolTip(tr("zsbercoin is currently disabled due to maintenance."));

        // Spend zsbercoin
        ui->pushButtonSpendzsbercoin->setEnabled(false);
        ui->pushButtonSpendzsbercoin->setToolTip(tr("zsbercoin is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zsbercoin
        ui->pushButtonMintzsbercoin->setEnabled(true);
        ui->pushButtonMintzsbercoin->setToolTip(tr("PrivacyDialog", "Enter an amount of sbercoin to convert to zsbercoin", 0));

        // Spend zsbercoin
        ui->pushButtonSpendzsbercoin->setEnabled(true);
        ui->pushButtonSpendzsbercoin->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
