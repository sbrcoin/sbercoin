...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered sbercoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 sbercoin.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal sbercoin:URI (sbercoin:XyZ...?message=example)
            {
...
static void NotifyzsbercoinReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzsbercoinReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzsbercoinReset.connect(boost::bind(NotifyzsbercoinReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzsbercoinReset.disconnect(boost::bind(NotifyzsbercoinReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsbercoinTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
