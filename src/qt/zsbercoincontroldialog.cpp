...
// Copyright (c) 2017-2018 The sbercoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zsbercoincontroldialog.h"
#include "ui_zsbercoincontroldialog.h"
...
std::set<std::string> ZsbercoinControlDialog::setSelectedMints;
std::set<CMintMeta> ZsbercoinControlDialog::setMints;

ZsbercoinControlDialog::ZsbercoinControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZsbercoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZsbercoinControlDialog::~ZsbercoinControlDialog()
{
    delete ui;
}

void ZsbercoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZsbercoinControlDialog::updateList()
{
...
void ZsbercoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZsbercoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZsbercoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

}

std::vector<CMintMeta> ZsbercoinControlDialog::GetSelectedMints()
{
...
void ZsbercoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
