...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::sbercoin);
    ...
    if (!settings.contains("nAnonymizesbercoinAmount"))
        settings.setValue("nAnonymizesbercoinAmount", 1000);

    nAnonymizesbercoinAmount = settings.value("nAnonymizesbercoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizesbercoinAmount"))
        SoftSetArg("-anonymizesbercoinamount", settings.value("nAnonymizesbercoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sbercoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizesbercoinAmount:
            return QVariant(nAnonymizesbercoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizesbercoinAmount:
            nAnonymizesbercoinAmount = value.toInt();
            settings.setValue("nAnonymizesbercoinAmount", nAnonymizesbercoinAmount);
            emit anonymizesbercoinAmountChanged(nAnonymizesbercoinAmount);
            break;
        case CoinControlFeatures:
