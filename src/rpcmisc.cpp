...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total sbercoin balance of the wallet \n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in sbercoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in sbercoin/kb\n"
    ...
    UniValue zsbercoinObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zsbercoinObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"sbercoinaddress\"\n"
            "\nReturn information about the given sbercoin address.\n"

            "\nArguments:\n"
            "1. \"sbercoinaddress\"     (string, required) The sbercoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"sbercoinaddress\", (string) The sbercoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: sbercoin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are sbercoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) sbercoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"sbercoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"sbercoinaddress\"  (string, required) The sbercoin address to use for the signature.\n"
