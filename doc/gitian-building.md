...
*Setup instructions for a gitian build of sbercoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the sbercoin
...
- [Building sbercoin](#building-sbercoin)
...
Clone the git repositories for sbercoin and gitian and then checkout the sbercoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sbercoin repository
...
To build sbercoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the sbercoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sbercoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/forking-altcoins/sbercoin
...
For example:
```bash
URL=https://github.com/crowning-/sbercoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit sbercoin=${COMMIT} --url sbercoin=${URL} ../sbercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sbercoin=${COMMIT} --url sbercoin=${URL} ../sbercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sbercoin=${COMMIT} --url sbercoin=${URL} ../sbercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sbercoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sbercoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/sbercoin-build.assert
```
...
[sbercoin/gitian.sigs](https://github.com/forking-altcoins-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
